...
export const Foreground = ({
  ...
  const [startTitleFade, finishTitleFade] = [scrollPosition(height, 25), scrollPosition(height, 45)];
  const imageOpacityAnimatedStyle = useAnimatedStyle(() => {
    return {
      opacity: interpolate(scrollValue.value, [0, startImgFade, finishImgFade], [1, 1, 0], Extrapolate.CLAMP)
    };
  }, [scrollValue, startImgFade, finishImgFade]);
  const imageAnimatedStyle = useAnimatedStyle(() => {
    const imageSize = interpolate(scrollValue.value, [0, startImgSize, finishImgSize], [startSize, startSize, endSize], Extrapolate.CLAMP);
    return {
  ...
  const titleAnimatedStyle = useAnimatedStyle(() => {
    return {
      opacity: interpolate(scrollValue.value, [0, startTitleFade, finishTitleFade], [1, 1, 0], Extrapolate.CLAMP)
    };
  ...
  }, foregroundImage ? /*#__PURE__*/React.createElement(Animated.View, {
